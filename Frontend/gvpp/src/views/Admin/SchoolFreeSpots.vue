...
import {useSchoolStore} from "@/store/SchoolStore.ts";
import {useSchoolRoomStore} from "@/store/SchoolRoomStore.ts";

const filterStore = useFilterStore();
const schoolStore = useSchoolStore();
const roomStore = useSchoolRoomStore();

const schoolsData = ref<Branch[]>([]);
const filters = ref<Filter[]>([]);
const showAddModal = ref(false);
const newSchool = ref<BranchRequest>();
...
const toggleExpand = (name: string) => {
  ...
  const room = schoolsData.value.find(er => er.name === name);
  if (room) {
...
async function applyFilters() {
  await getSchoolsFromBackend();

  schoolsData.value = schoolsData.value.filter((school) => {
    return filters.value.every((filter) => {
      if (filter.name === "City") {
        return school.city.includes(filter.value);
      }
  ...
  schoolsData.value.forEach((school) => {
    school.rooms = school.rooms.filter((room) => {
      return filters.value.every((filter) => {
  ...
  schoolsData.value = schoolsData.value.filter((school) => school.rooms.length > 0);
}
...
async function getSchoolsFromBackend() {
  schoolsData.value = await schoolStore.getSchools();
  schoolsData.value = schoolsData.value.sort((a, b) => a.id - b.id);
}
...
async function saveCapacity(roomId: number) {
  ...
  try {
    await roomStore.updateSchoolRoom(roomId, request);
    await getSchoolsFromBackend(); // Refresh data
    toggleEdit(roomId);
  } catch (error) {
...
async function deleteSchoolRoom(roomId: number): Promise<void> {
  await roomStore.deleteSchoolRoom(roomId);
}

async function addSchool(): Promise<void> {
  try {
    if (newSchool.value) {
      await schoolStore.addSchool(newSchool.value);
      await getSchoolsFromBackend();
      closeAddModal();
    }
  } catch (error) {
...
<template>
  ...
  <div class="bg-slate-200 min-h-screen p-4">
    ...
    <div class="w-full mt-10 mb-5 grid gap-6">
      <div v-for="escaperoom in schoolsData" :key="escaperoom.id"
        ...
        <div v-if="expandedCards[escaperoom.name]" class="mt-4">
          ...
          >
            ...
            <button
              ...
              @click="deleteSchoolRoom(room.id)"
              title="Verwijder"
            >
              🗑️
            </button>
    ...
    <div
      ...
      <div class="bg-white rounded-lg p-6 w-full max-w-md my-10 max-h-[90vh] shadow-lg overflow-y-auto">
        ...
        <div class="mb-4">
          ...
          <input type="text" v-model="newSchool!.name" class="w-full border rounded px-3 py-2" />
        </div>
        <div class="mb-4">
          <label class="block font-semibold mb-1">Description</label>
          <textarea v-model="newSchool!.description" class="w-full border rounded px-3 py-2 h-30" />
        </div>
        <div class="mb-4">
          <label class="block font-semibold mb-1">Organisation-Id</label>
          <input type="number" v-model.number="newSchool!.organizationId" min="1" class="w-20 border rounded px-3 py-2" />
        </div>
        <div class="mb-4">
          <label class="block font-semibold mb-1">Address</label>
          <textarea v-model="newSchool!.address" class="w-full border rounded px-3 py-2 h-30" />
        </div>
        <div class="mb-4">
          <label class="block font-semibold mb-1">Postal Code</label>
          <input type="number" v-model.number="newSchool!.postalCode" min="1" class="w-20 border rounded px-3 py-2" />
        </div>
        <div class="mb-4">
          <label class="block font-semibold mb-1">City</label>
          <textarea v-model="newSchool!.city" class="w-full border rounded px-3 py-2 h-30" />
        </div>
        <div class="mb-4">
          <label class="block font-semibold mb-1">Website</label>
          <textarea v-model="newSchool!.website" class="w-full border rounded px-3 py-2 h-30" />
        </div>
        <div class="mb-4">
          <label class="block font-semibold mb-1">Phone number</label>
          <textarea v-model="newSchool!.phoneNumber" class="w-full border rounded px-3 py-2 h-30" />
        </div>

        <div class="flex justify-end gap-2">
          <button @click="closeAddModal" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Annuleer</button>
          <button @click="addSchool" class="px-4 py-2 bg-blue-800 text-white rounded hover:bg-blue-900">Opslaan</button>
        </div>
      </div>
    </div>
  </div>
