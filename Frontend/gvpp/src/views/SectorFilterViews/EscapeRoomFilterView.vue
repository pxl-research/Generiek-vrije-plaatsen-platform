...
const escapeRoomsData = ref<Branch[]>([]);
const filters = ref<Filter[]>([]);
const filteredEscapeRooms = computed(() => escapeRoomsData.value);
...
async function applyFilters() {
  ...
  escapeRoomsData.value = escapeRoomsData.value.filter((escaperoom) => {
  ...
  escapeRoomsData.value.forEach((escaperoom) => {
  ...
  escapeRoomsData.value = escapeRoomsData.value.filter((escaperoom) => escaperoom.rooms.length > 0);
}
